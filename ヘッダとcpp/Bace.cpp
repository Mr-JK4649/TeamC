...
void Base_Disp(int width, int height) {
	...
	if (menu.isMenu == true) { menu.Draw(); menu.Item_Kind(Return_Item(menu.item_select)); Status_Disp(); }			//メニューのフラグがtrueだったらメニューとステータスの表示
...
void Base_Update(int width, int height) {
	...
	/*キャラの移動など*/
	if (!menu.isMenu && !str.setTex)Chara_Update();
	...
	if (!str.setTex && inp.start) menu.isMenu = true;		//文字ウィンドウが出てないときにスタートボタンを押すと、メニューフラグの切り替え
