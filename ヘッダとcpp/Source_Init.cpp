...
void Init() {
	...
	if(Loading_Par != 90.0f)
		str.SuperString(w100 * 50, h100 * 40, "“Ç‚Ýž‚Ý‚ñ‚®!", 0xffffff, 1, size * 2);
	DrawBox(w100 * 5, h100 * 55, w100 * 5 + w100 * Loading_Par, h100 * 60, 0x00ff00, true);
	DrawBox(w100 * 5, h100 * 55, w100 * 95, h100 * 60, 0xffff00, false);
	...
	else if (ch.flg) { ch.Init(&ch); ch.flg = false; Loading_Par = 90.0f; }

	if (Loading_Par >= 90.0f) {
		str.SuperString(w100 * 50, h100 * 40, "“Ç‚Ýž‚ÝŠ®—¹!", 0xffffff, 1, size * 2);
		if(++count >= 100)g_GameState = GAME_BASE;
	}
}